    ...
    >
      <div className="container  bg-primary   w-auto  mt-8 mx-auto flex flex-row justify-between items-center ">
        <div className="flex justify-between items-center space-x-3">
          ...
          <Typography variant="body2" sx={{ border: '2px solid #0A7225', color: '#0BFF48', px: 2, mt: '6px', fontSize: '14px' }}>
            Aplha
          </Typography>
        </div>
        ...
        <div className="flex justify-between items-center text-white text-lg opacity-95  space-x-8    ">
          {!props.isAuthenticated && props.active !== 'SIGN_UP' ? (
            <a className=" hover:text-secondary  transition ease-in duration-150" href="/sign-up">
              Sign up
            </a>
          ) : null}

          {!props.isAuthenticated && props.active !== 'SIGN_IN' ? (
            <a className=" hover:text-secondary  transition ease-in duration-150" href="/sign-in">
              Sign in
            </a>
          ) : null}

          {props.isAuthenticated && props.active === 'INDEX' ? (
            <a className=" hover:text-secondary  transition ease-in duration-150" href="/home">
              Sign in
            </a>
          ) : null}

          <a className="hover:text-secondary hidden  transition ease-in duration-150  sm:block " href="/verify-cid">
            Verify
          </a>

          <a className=" hover:text-secondary  hidden transition ease-in duration-150 sm:block" href="https://docs.estuary.tech">
            Documentation
          </a>

          {props.isAuthenticated ? (
            <a className=" hover:text-secondary  transition ease-in duration-150" href="/_">
              Menu
            </a>
          ) : null}
        </div>
